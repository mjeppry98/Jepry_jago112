...
-- Fungsi untuk show/hide semua elemen UI utama
local function setUIVisible(visible)
    uiVisible          = visible
    Window.Visible     = visible
    Shadow.Visible     = visible and not minimized
...
UserInputService.InputChanged:Connect(function(inp)
    ...
    if dragging then
        if isMouse then
            -- PC: hitung delta dari posisi awal drag (akurat & stabil)
            ...
            -- Mobile: gunakan inp.Delta (perubahan per-frame)
            -- Metode ini tidak terpengaruh oleh perpindahan jari antar UI element
            local newX = Window.Position.X.Offset + inp.Delta.X
            local newY = Window.Position.Y.Offset + inp.Delta.Y
            Window.Position = UDim2.new(Window.Position.X.Scale, newX, Window.Position.Y.Scale, newY)
            Shadow.Position = UDim2.new(Window.Position.X.Scale, newX - 10, Window.Position.Y.Scale, newY - 4)
        end
    end
    ...
    if tDragging then
        if isMouse and tDragStart and tBtnStart then
            ...
            local newX  = tBtnStart.X.Offset + delta.X
            local newY  = tBtnStart.Y.Offset + delta.Y
            ToggleBtn.Position    = UDim2.new(tBtnStart.X.Scale, newX, tBtnStart.Y.Scale, newY)
            ToggleShadow.Position = UDim2.new(tBtnStart.X.Scale, newX - 5, tBtnStart.Y.Scale, newY + 5)
        elseif isTouch then
            local newBX = ToggleBtn.Position.X.Offset + inp.Delta.X
            local newBY = ToggleBtn.Position.Y.Offset + inp.Delta.Y
            local newSX = ToggleShadow.Position.X.Offset + inp.Delta.X
            local newSY = ToggleShadow.Position.Y.Offset + inp.Delta.Y
            ToggleBtn.Position    = UDim2.new(ToggleBtn.Position.X.Scale, newBX, ToggleBtn.Position.Y.Scale, newBY)
            ToggleShadow.Position = UDim2.new(ToggleShadow.Position.X.Scale, newSX, ToggleShadow.Position.Y.Scale, newSY)
        end
    end
