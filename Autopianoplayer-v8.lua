-- ╔══════════════════════════════════════════════════════════════════════════════
-- ║  🔑  KONFIGURASI API KEYS — ISI KEY ANDA DI SINI                           ║
-- ║  Ganti "PASTE_..._DI_SINI" dengan key asli Anda.                           ║
-- ║  🟡 Groq     → console.groq.com          → API Keys → Create API Key      ║
-- ║  🔵 Gemini   → aistudio.google.com        → Get API key                    ║
-- ║  🟣 OpenRtr  → openrouter.ai              → Keys → Create key             ║
-- ║  🔴 YT API   → console.cloud.google.com   → YouTube Data API v3           ║
-- ╚══════════════════════════════════════════════════════════════════════════════
local CONFIG_API = {
    groq       = "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",        -- 🟡 WAJIB
    gemini     = "AQ.Ab8RN6JeTcmDBbyG7O2h7wOIHWNSfqvmMQn3ZnJPq0S4Sm4-ng",       -- 🔵 Opsional (backup)
    openrouter = "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",   -- 🟣 Opsional (backup 2)
    ytdata     = "AIzaSyC6NHJh7nJ4bhg0zVvv0IeFfbs3SjoZfTY",  -- 🔴 Opsional
}
-- ══════════════════════════════════════════════════════════════════════════════
...
-- ╔══════════════════════════════════════════════════════════════════════════════
-- ║  ToggleWrapper: container transparan yang membungkus shadow + button.       ║
-- ║  Saat drag, cukup pindahkan WRAPPER → shadow & button ikut otomatis.       ║
-- ║  Tidak ada lagi sync manual = tidak ada shadow bug.                         ║
-- ╚══════════════════════════════════════════════════════════════════════════════
local WRAP_W = TOGGLE_W + 12   -- wrapper lebih lebar dari button (ruang shadow)
local WRAP_H = TOGGLE_H + 10   -- wrapper lebih tinggi dari button

local ToggleWrapper = newFrame(ScreenGui, {
    Name                   = "ToggleWrapper",
    -- Posisi awal: pojok kanan atas, 4px dari tepi kanan, 8px dari tepi atas
    Size                   = UDim2.new(0, WRAP_W, 0, WRAP_H),
    Position               = UDim2.new(1, -(WRAP_W + 4), 0, 8),
    BackgroundTransparency = 1,   -- wrapper sendiri tidak terlihat
    ZIndex                 = 24,
    Active                 = false,
    ClipsDescendants       = false,  -- biarkan shadow overflow ke luar wrapper
})

-- Shadow: child pertama dari wrapper (ZIndex lebih rendah = render di belakang button)
local ToggleShadow = newFrame(ToggleWrapper, {
    Name                   = "ToggleShadow",
    Size                   = UDim2.new(0, TOGGLE_W, 0, TOGGLE_H),
    Position               = UDim2.new(0, 6, 0, 6),   -- geser 6px kanan-bawah = efek shadow
    BackgroundColor3       = Color3.fromRGB(0, 0, 0),
    BackgroundTransparency = 0.55,
    ZIndex                 = 24,   -- lebih rendah dari button (25) → di belakang
})
corner(ToggleShadow, 12)

-- Toggle button: child kedua dari wrapper (ZIndex lebih tinggi = di depan shadow)
local ToggleBtn = newBtn(ToggleWrapper, {
    Name             = "ToggleButton",
    Size             = UDim2.new(0, TOGGLE_W, 0, TOGGLE_H),
    Position         = UDim2.new(0, 0, 0, 0),   -- pojok kiri-atas wrapper
    BackgroundColor3 = C.SURFACE2,
    ZIndex           = 25,
    Text             = "",
    Active           = true,
...
-- Simpan posisi WRAPPER (bukan button) sebagai referensi, karena yang digeser adalah wrapper
ToggleBtn.InputBegan:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1
    or inp.UserInputType == Enum.UserInputType.Touch then
        tDragging  = true
        tDragStart = inp.Position
        tBtnStart  = ToggleWrapper.Position   -- posisi wrapper (UDim2 dengan Scale X=1)
    end
end)
...
-- ╔══════════════════════════════════════════════════════════════════════════════
-- ║   KEYBOARD VISUALIZER  (akurat 1:1 — setiap key = slot sendiri)            ║
-- ║   Layout persis keyboard QWERTY + angka, sama seperti yang dipakai         ║
-- ║   di sheet Sky Music. Tidak ada mapping — langsung cocok.                  ║
-- ╚══════════════════════════════════════════════════════════════════════════════

-- 4 baris keyboard persis seperti layout di sheet Sky Music
local KB_ROWS = {
    {"1","2","3","4","5","6","7","8","9","0"},  -- baris angka (10 key)
    {"q","w","e","r","t","y","u","i","o","p"},  -- baris atas  (10 key)
    {"a","s","d","f","g","h","j","k","l"},       -- baris tengah (9 key)
    {"z","x","c","v","b","n","m"},               -- baris bawah  (7 key)
}
-- Warna default tiap baris (sedikit berbeda agar mudah dibedakan)
local KB_ROW_COLOR = {
    Color3.fromRGB(38, 38, 48),   -- baris angka: biru-gelap
    Color3.fromRGB(36, 42, 36),   -- baris atas: hijau-gelap
    Color3.fromRGB(42, 36, 36),   -- baris tengah: merah-gelap
    Color3.fromRGB(42, 40, 30),   -- baris bawah: coklat-gelap
}
local KB_ROW_LIT = {
    Color3.fromRGB(80, 120, 220),  -- angka → biru
    C.GOLD,                         -- atas → gold
    C.GREEN,                        -- tengah → hijau
    Color3.fromRGB(220, 160, 60),  -- bawah → oranye
}

-- Hitung ukuran key berdasarkan WIN_W
-- Baris terpanjang = 10 key → jadikan acuan
local VIS_INNER   = WIN_W - 28 - 8          -- lebar dalam VisFrame (dikurangi padding dan margin)
local KB_GAP      = math.max(2, math.floor(VIS_INNER / 120))   -- gap antar key (2-4px)
local KEY_W       = math.floor((VIS_INNER - 9 * KB_GAP) / 10)  -- lebar 1 key
local KEY_H       = math.max(18, math.floor(KEY_W * 0.55))      -- tinggi key (proporsional)
local KEY_FONT_SZ = math.max(8, math.floor(KEY_H * 0.5))        -- ukuran font

-- Total tinggi visualizer = 4 baris + padding atas-bawah
local VIS_H = 4 * KEY_H + 3 * KB_GAP + 12

local VisFrame = newFrame(ContentPad, {
    Name             = "Visualizer",
    Size             = UDim2.new(1, 0, 0, VIS_H),
    BackgroundColor3 = C.SURFACE2,
...
-- Tabel utama: keyChar (string) → {frame, labelFrame, rowIdx}
local keyFrameMap = {}

for rowIdx, rowKeys in ipairs(KB_ROWS) do
    local nKeys    = #rowKeys
    local rowW     = nKeys * KEY_W + (nKeys - 1) * KB_GAP
    local rowLeft  = math.floor((VIS_INNER - rowW) / 2) + 4  -- center per baris
    local rowTop   = 6 + (rowIdx - 1) * (KEY_H + KB_GAP)
    local defColor = KB_ROW_COLOR[rowIdx]

    for colIdx, keyChar in ipairs(rowKeys) do
        local kx = rowLeft + (colIdx - 1) * (KEY_W + KB_GAP)

        -- Frame background key
        local kf = newFrame(VisFrame, {
            Size             = UDim2.new(0, KEY_W, 0, KEY_H),
            Position         = UDim2.new(0, kx, 0, rowTop),
            BackgroundColor3 = defColor,
            ZIndex           = 13,
        })
        corner(kf, math.max(3, math.floor(KEY_W * 0.12)))

        -- Label huruf
        local lbl = newLabel(kf, {
            Text       = keyChar:upper(),
            Size       = UDim2.new(1, 0, 1, -2),
            Position   = UDim2.new(0, 0, 0, 1),
            Font       = Enum.Font.GothamBold,
            TextSize   = KEY_FONT_SZ,
        ...
        keyFrameMap[keyChar] = {
            ...
            rowIdx   = rowIdx,
            defColor = defColor,
            litColor = KB_ROW_LIT[rowIdx],
        }
    end
end

-- ── LIGHT FUNCTION (langsung cocok, tanpa mapping) ────────────────────────────
-- skyKeys = tabel karakter yang sedang ditekan, misal {"e","u","p"}
-- Fungsi ini HANYA menyalakan key yang sesuai — tidak ada lookup table.
local function lightVisKeys(skyKeys)
    -- Reset semua key ke warna default
    for _, kd in pairs(keyFrameMap) do
        kd.frame.BackgroundColor3 = kd.defColor
        kd.label.TextColor3       = Color3.fromRGB(90, 90, 90)
    end
    if not skyKeys or #skyKeys == 0 then return end

    for _, k in ipairs(skyKeys) do
        local kd = keyFrameMap[k:lower()]
        if kd then
            -- Nyalakan key
            kd.frame.BackgroundColor3 = kd.litColor
            kd.label.TextColor3       = Color3.fromRGB(15, 15, 15)
            -- Auto-redam setelah durasi note
            task.delay(0.18, function()
                if kd.frame and kd.frame.Parent then
                    tween(kd.frame, { BackgroundColor3 = kd.defColor }, 0.10)
                    kd.label.TextColor3 = Color3.fromRGB(90, 90, 90)
                end
            end)
        end
...
local PlayBtn,  PlayLbl,  PlayIcon  = makeBtn(BtnRow, "PLAY",  "▶️", C.GOLD,     C.BLACK, 1)
...
-- ║   🔑 API KEYS SECTION                                                       ║
-- ╚══════════════════════════════════════════════════════════════════════════════
sectionLabel(ContentPad, "🔑  API KEYS", 21)

-- Storage API keys: prioritas → CONFIG_API (hardcoded) → file → UI input
-- Fungsi ini abaikan teks placeholder "PASTE_..._DI_SINI"
local function resolveKey(v)
    if type(v) ~= "string" or v == "" or v:find("^PASTE_") then return "" end
    return v
end
local apiKeys = {
    groq       = resolveKey(CONFIG_API.groq),
    gemini     = resolveKey(CONFIG_API.gemini),
    openrouter = resolveKey(CONFIG_API.openrouter),
    ytdata     = resolveKey(CONFIG_API.ytdata),
}

local function saveApiKeysToFile()
    if not writefile then return false end
    local ok = pcall(writefile, "autpiano_keys.json",
        game:GetService("HttpService"):JSONEncode(apiKeys))
    return ok
end
local function loadApiKeysFromFile()
    if not readfile then return end
    local ok, raw = pcall(readfile, "autpiano_keys.json")
    if not ok or not raw then return end
    local ok2, data = pcall(game:GetService("HttpService").JSONDecode,
                             game:GetService("HttpService"), raw)
    if ok2 and data then
        -- File overrides CONFIG_API hanya jika file punya value (non-empty)
        if data.groq       and data.groq       ~= "" then apiKeys.groq       = data.groq       end
        if data.gemini     and data.gemini     ~= "" then apiKeys.gemini     = data.gemini     end
        if data.openrouter and data.openrouter ~= "" then apiKeys.openrouter = data.openrouter end
        if data.ytdata     and data.ytdata     ~= "" then apiKeys.ytdata     = data.ytdata     end
    end
end
loadApiKeysFromFile()

local ApiPanel = newFrame(ContentPad, {
    Size             = UDim2.new(1, 0, 0, 0),
    AutomaticSize    = Enum.AutomaticSize.Y,
    BackgroundColor3 = C.SURFACE2,
    LayoutOrder      = 22,
...
listLayout(ApiPanel, 4)

local apiInputRefs = {}
local function makeKeyRow(parent, emoji, name, hint, keyId, order)
    local row = newFrame(parent, {
    ...
    corner(box, 5); stroke(box, C.BORDER)
    box:GetPropertyChangedSignal("Text"):Connect(function()
        apiKeys[keyId] = box.Text
    end)
    apiInputRefs[keyId] = box
    return row
end
makeKeyRow(ApiPanel, "🟡", "Groq",       "sk-...  (wajib, AI utama)",          "groq",       1)
makeKeyRow(ApiPanel, "🔵", "Gemini",     "AIza...  (backup AI, opsional)",      "gemini",     2)
makeKeyRow(ApiPanel, "🟣", "OpenRouter", "sk-or-...  (opsional, model gratis)", "openrouter", 3)
makeKeyRow(ApiPanel, "🔴", "YT Data",    "AIza...  (opsional)",                 "ytdata",     4)

-- Save button row
local apiActRow = newFrame(ApiPanel, {
    Size                   = UDim2.new(1, 0, 0, 28),
    BackgroundTransparency = 1,
    LayoutOrder            = 5,
    ZIndex                 = 13,
})
local apiSaveBtn = newBtn(apiActRow, {
    Text             = "💾  Save Keys",
...
corner(apiSaveBtn, 6)
local apiSaveStatus = newLabel(apiActRow, {
    Text           = "Keys tersimpan di sesi ini",
...
apiSaveBtn.MouseButton1Click:Connect(function()
    local ok = saveApiKeysToFile()
    apiSaveStatus.Text       = ok and "✅ Disimpan ke file!" or "⚠ writefile tidak tersedia di executor ini"
    apiSaveStatus.TextColor3 = ok and C.GREEN or C.GOLD
    task.delay(3, function()
        apiSaveStatus.Text       = "Keys tersimpan di sesi ini"
        apiSaveStatus.TextColor3 = C.TEXT_MUT
    end)
end)

-- Info bar
local apiInfoRow = newFrame(ApiPanel, {
    Size                   = UDim2.new(1, 0, 0, 0),
    AutomaticSize          = Enum.AutomaticSize.Y,
    BackgroundTransparency = 1,
    LayoutOrder            = 6,
...
newLabel(apiInfoRow, {
    Text = "📌 Gratis:  🟡 console.groq.com  ·  🔵 aistudio.google.com  ·  🟣 openrouter.ai  ·  🔴 console.cloud.google.com",
    Size          = UDim2.new(1, 0, 0, 0),
    AutomaticSize = Enum.AutomaticSize.Y,
    Font          = Enum.Font.Gotham,
    TextSize      = 8,
...
-- ║   🎬 SHEET MAKER SECTION                                                    ║
-- ╚══════════════════════════════════════════════════════════════════════════════
sectionLabel(ContentPad, "🎬  SHEET MAKER  ·  Video / Lagu → Sheet Otomatis", 23)

-- Input URL / nama lagu
local smInputFrame = newFrame(ContentPad, {
    Size             = UDim2.new(1, 0, 0, 44),
    BackgroundColor3 = C.SURFACE2,
    LayoutOrder      = 24,
    ZIndex           = 12,
})
corner(smInputFrame, 8); stroke(smInputFrame, C.BORDER)
newLabel(smInputFrame, {
    Text           = "🎵",
...
local SmInputBox = newBox(smInputFrame, {
    Text              = "",
    PlaceholderText   = "YouTube URL atau nama lagu... (contoh: Lemon Yonezu)",
    PlaceholderColor3 = C.TEXT_MUT,
...
corner(SmInputBox, 6); stroke(SmInputBox, C.BORDER)

-- AI model selector row
local smModelRow = newFrame(ContentPad, {
    Size                   = UDim2.new(1, 0, 0, 34),
    BackgroundTransparency = 1,
    LayoutOrder            = 25,
    ZIndex                 = 12,
})
local smModelLL = Instance.new("UIListLayout")
smModelLL.FillDirection = Enum.FillDirection.Horizontal
smModelLL.Padding       = UDim.new(0, 6)
smModelLL.SortOrder     = Enum.SortOrder.LayoutOrder
smModelLL.Parent        = smModelRow

local selectedModel = "auto"   -- "groq" | "gemini" | "auto"
local modelBtns = {}
local MODEL_DEFS = {
    { id="groq",   label="🟡 Groq",   desc="Llama 3.3 70B" },
    { id="gemini", label="🔵 Gemini", desc="Flash 1.5" },
    { id="auto",   label="⚡ Auto",   desc="Coba Groq → Gemini" },
}
for i, md in ipairs(MODEL_DEFS) do
    local mb = newBtn(smModelRow, {
        Text             = md.label,
        Size             = UDim2.new(0.333, -5, 1, 0),
        BackgroundColor3 = md.id == "auto" and C.GOLD or C.SURFACE3,
        Font             = Enum.Font.GothamBold,
        TextSize         = 9,
        TextColor3       = md.id == "auto" and C.BLACK or C.TEXT_DIM,
        LayoutOrder      = i,
        ZIndex           = 13,
    })
    corner(mb, 7)
    if md.id ~= "auto" then stroke(mb, C.BORDER) end
    modelBtns[md.id] = mb
end
local function selectModel(id)
    selectedModel = id
    for mid, mb in pairs(modelBtns) do
        if mid == id then
            mb.BackgroundColor3 = C.GOLD
            mb.TextColor3       = C.BLACK
            -- remove stroke
            for _, s in ipairs(mb:GetChildren()) do
                if s:IsA("UIStroke") then s:Destroy() end
            end
        else
            mb.BackgroundColor3 = C.SURFACE3
            mb.TextColor3       = C.TEXT_DIM
            if not mb:FindFirstChildOfClass("UIStroke") then
                stroke(mb, C.BORDER)
            end
        end
    end
...
for mid, mb in pairs(modelBtns) do
    mb.MouseButton1Click:Connect(function() selectModel(mid) end)
end

-- Generate button
local smGenRow = newFrame(ContentPad, {
    Size                   = UDim2.new(1, 0, 0, 36),
    BackgroundTransparency = 1,
    LayoutOrder            = 26,
    ZIndex                 = 12,
})
local SmGenBtn = newBtn(smGenRow, {
    Text             = "✨  Generate Sheet",
    Size             = UDim2.new(1, 0, 1, 0),
...
corner(SmGenBtn, 8)
local smGenStatus = newLabel(smGenRow, {   -- overlay status text saat loading
    Text      = "",
    Size      = UDim2.new(1, 0, 1, 0),
    Font      = Enum.Font.Gotham,
    TextSize  = 10,
...
-- Output text area
local smOutFrame = newFrame(ContentPad, {
...
corner(smOutFrame, 8); stroke(smOutFrame, C.BORDER)
local smOutHeader = newLabel(smOutFrame, {
    Text           = "📄 Output Sheet",
    Size           = UDim2.new(1, 0, 0, 18),
...
local smOutBox = newBox(smOutFrame, {
    Text              = "",
    PlaceholderText   = "Sheet yang dihasilkan AI akan muncul di sini...",
    PlaceholderColor3 = C.TEXT_MUT,
...
-- Use Sheet button
local smUseRow = newFrame(ContentPad, {
    Size                   = UDim2.new(1, 0, 0, 36),
    BackgroundTransparency = 1,
    LayoutOrder            = 28,
    ZIndex                 = 12,
})
local SmUseBtn = newBtn(smUseRow, {
    Text             = "▶️  Gunakan Sheet Ini  →  Player",
    Size             = UDim2.new(1, 0, 1, 0),
    BackgroundColor3 = C.GOLD,
    Font             = Enum.Font.GothamBold,
    TextSize         = 11,
...
SmUseBtn.MouseButton1Click:Connect(function()
    local sheet = smOutBox.Text
    if not sheet or sheet == "" then
        notify("Sheet Kosong", "Generate sheet dulu sebelum menggunakannya.", 3, "warn")
        return
    end
    SheetBox.Text = sheet
    local toks = parseSheet(sheet)
    updateStats(toks)
    notify("Sheet Siap!", "Sheet berhasil disalin ke player. Tekan Play!", 4, "success")
    addLog("🎬 Sheet dari Sheet Maker dipakai di player ✓", "info")
    -- Scroll ke atas agar user lihat sheet input
...
UserInputService.InputChanged:Connect(function(inp)
    ...
    -- ── Toggle button drag (geser ToggleWrapper → shadow & button ikut sekaligus) ──
    -- Tidak perlu update dua frame secara terpisah → tidak ada shadow desync bug
    if tDragging then
        local vpS = workspace.CurrentCamera.ViewportSize
        local rawX, rawY
        if isMouse and tDragStart and tBtnStart then
            local delta = inp.Position - tDragStart
            rawX = tBtnStart.X.Offset + delta.X
            rawY = tBtnStart.Y.Offset + delta.Y
        elseif isTouch then
            rawX = ToggleWrapper.Position.X.Offset + inp.Delta.X
            rawY = ToggleWrapper.Position.Y.Offset + inp.Delta.Y
        end
        if rawX and rawY then
            -- Clamp berdasarkan ukuran wrapper agar tidak keluar layar
            local clX = math.clamp(rawX, -(vpS.X - 4), -(WRAP_W + 4))
            local clY = math.clamp(rawY, 4, vpS.Y - WRAP_H - 4)
            ToggleWrapper.Position = UDim2.new(1, clX, 0, clY)
            -- Shadow dan button mengikuti otomatis (child dari wrapper)
        end
    end
...
-- ╔══════════════════════════════════════════════════════════════════════════════
-- ║   🌐 SHEET MAKER BACKEND                                                    ║
-- ║   HTTP wrapper → YouTube title → Groq → Gemini → Generate handler          ║
-- ╚══════════════════════════════════════════════════════════════════════════════

local HttpService = game:GetService("HttpService")

-- ── HTTP Request Wrapper (kompatibel dengan executor paling umum) ─────────────
local function httpReq(url, method, headers, body)
    ...
    local res
    if request       then res = pcall(request, opts)       and request(opts)
    elseif syn and syn.request then
        local ok, r = pcall(syn.request, opts); if ok then res = r end
    elseif http_request then
        local ok, r = pcall(http_request, opts); if ok then res = r end
    elseif KRNL_LOADED and http then
        local ok, r = pcall(http, opts); if ok then res = r end
    end
    if not res then
        return nil, "❌ Executor ini tidak support HTTP request"
    end
    if res.StatusCode and res.StatusCode >= 400 then
        return nil, string.format("❌ HTTP %d: %s", res.StatusCode, tostring(res.Body):sub(1, 120))
    end
    return res.Body or res.body, nil
end

-- ── YouTube: ekstrak Video ID dari berbagai format URL ────────────────────────
local function extractVideoId(url)
    return url:match("[?&]v=([%w%-_]+)")
           or url:match("youtu%.be/([%w%-_]+)")
           or url:match("/shorts/([%w%-_]+)")
           or url:match("/embed/([%w%-_]+)")
end

-- Ambil judul video dari YouTube oEmbed (GRATIS, tidak perlu API key)
local function getVideoTitle(videoUrl)
    local encoded = HttpService:UrlEncode(videoUrl)
    local body, err = httpReq(
        "https://www.youtube.com/oembed?url=" .. encoded .. "&format=json",
        "GET"
    )
    if not body then return nil, err end
    local ok, data = pcall(HttpService.JSONDecode, HttpService, body)
    if ok and data and data.title then return data.title, nil end
    return nil, "Gagal membaca judul video"
end

-- Prompt sistem untuk generasi sheet
local function buildPrompt(songName)
    return string.format([[
You are a Virtual Piano sheet music transcriber for the Roblox game "Virtual Piano" / Sky Music.

Song to transcribe: "%s"

STRICT RULES — follow exactly:
1. Only use these keys: 1 2 3 4 5 6 7 8 9 0 q w e r t y u i o p a s d f g h j k l z x c v b n m
2. Chords (notes played AT THE SAME TIME) go inside square brackets: [eup] [rua] [48qepj]
3. Single notes have NO brackets: u f p t e
4. Separate each note/chord with a single space
5. Use | to mark measure/bar lines
6. Make it at least 80 tokens long — cover the main melody and repeat if needed
7. Output ONLY the raw sheet. No title, no explanation, no markdown, no commentary.

Sheet:]], songName)
end

-- ── Groq API (Llama 3.3 70B) — GRATIS, 30 req/min ────────────────────────────
local function generateWithGroq(songName)
    if not apiKeys.groq or apiKeys.groq == "" then
        return nil, "Groq API key belum diset. Masukkan di bagian 🔑 API KEYS."
    end
    local payload = HttpService:JSONEncode({
        model       = "llama-3.3-70b-versatile",
        messages    = {{ role = "user", content = buildPrompt(songName) }},
        max_tokens  = 1600,
        temperature = 0.25,
        top_p       = 0.9,
    })
    ...
    if not body then return nil, "Groq: " .. (err or "unknown error") end
    local ok, data = pcall(HttpService.JSONDecode, HttpService, body)
    if ok and data and data.choices and data.choices[1] then
        local text = data.choices[1].message and data.choices[1].message.content
        if text and text ~= "" then return text:match("^%s*(.-)%s*$"), nil end
    end
    if ok and data and data.error then
        return nil, "Groq error: " .. tostring(data.error.message or data.error)
    end
    return nil, "Groq: Gagal parse response"
end

-- ── Gemini API (Gemini 1.5 Flash) — GRATIS, 15 req/min ──────────────────────
local function generateWithGemini(songName)
    if not apiKeys.gemini or apiKeys.gemini == "" then
        return nil, "Gemini API key belum diset. Masukkan di bagian 🔑 API KEYS."
    end
    local payload = HttpService:JSONEncode({
        contents          = {{ parts = {{ text = buildPrompt(songName) }} }},
        generationConfig  = {
            temperature     = 0.25,
    ...
    if not body then return nil, "Gemini: " .. (err or "unknown error") end
    local ok, data = pcall(HttpService.JSONDecode, HttpService, body)
    if ok and data and data.candidates and data.candidates[1] then
    ...
    if ok and data and data.error then
        return nil, "Gemini error: " .. tostring(data.error.message or data.error.status)
    end
    return nil, "Gemini: Gagal parse response"
end

-- ── OpenRouter API (model gratis: Llama 3.1 8B, Mistral 7B, Gemma 2 9B) ─────
local function generateWithOpenRouter(songName)
    if not apiKeys.openrouter or apiKeys.openrouter == "" then
        return nil, "OpenRouter API key belum diset."
    end
    local payload = HttpService:JSONEncode({
    ...
    if not body then return nil, "OpenRouter: " .. (err or "unknown error") end
    local ok, data = pcall(HttpService.JSONDecode, HttpService, body)
    if ok and data and data.choices and data.choices[1] then
        local text = data.choices[1].message and data.choices[1].message.content
        if text and text ~= "" then return text:match("^%s*(.-)%s*$"), nil end
    ...
    if ok and data and data.error then
        return nil, "OpenRouter error: " .. tostring(data.error.message or data.error)
    end
    return nil, "OpenRouter: Gagal parse response"
end

-- ── Generate Button Handler ───────────────────────────────────────────────────
local smGenerating = false

SmGenBtn.MouseButton1Click:Connect(function()
    if smGenerating then return end

    local input = SmInputBox.Text:match("^%s*(.-)%s*$")
    if input == "" then
        notify("Input Kosong", "Masukkan YouTube URL atau nama lagu.", 3, "warn")
        return
    end

    smGenerating = true
    SmGenBtn.Visible    = false
    smGenStatus.Visible = true
    smOutBox.Text       = ""
    smOutBox.TextColor3 = C.TEXT_DIM

    -- ── Langkah 1: Deteksi apakah URL YouTube ────────────────────────────────
    local songName  = input
    local isYouTube = input:find("youtu") ~= nil

    if isYouTube then
        smGenStatus.Text = "📡 Mengambil judul video YouTube..."
        local vidId = extractVideoId(input)
        if vidId then
            local title, titleErr = getVideoTitle(input)
            if title then
                songName = title
                smGenStatus.Text = "🎵 Judul: " .. title:sub(1, 50)
                addLog("🎬 YouTube: " .. title, "info")
                task.wait(0.4)
            else
                songName = "video id " .. vidId
                addLog("⚠ Gagal ambil judul: " .. (titleErr or "?"), "warn")
            end
        else
            notify("URL Tidak Valid", "Tidak bisa baca video ID dari URL ini.", 3, "error")
            SmGenBtn.Visible    = true
            smGenStatus.Visible = false
            smGenerating        = false
            return
        end
    end

    -- ── Langkah 2: Generate sheet via AI ─────────────────────────────────────
    smGenStatus.Text = "✨ Membuat sheet: " .. songName:sub(1, 35) .. "..."
    addLog("🤖 Generating: " .. songName, "info")

    local sheet, genErr

    if selectedModel == "groq" then
        sheet, genErr = generateWithGroq(songName)

    elseif selectedModel == "gemini" then
        sheet, genErr = generateWithGemini(songName)

    else
        -- Auto: Groq → OpenRouter → Gemini (urutan kualitas & kecepatan)
        smGenStatus.Text = "🟡 Mencoba Groq..."
        sheet, genErr = generateWithGroq(songName)

        if not sheet and apiKeys.openrouter ~= "" then
            addLog("⚠ Groq: " .. (genErr or "?") .. " — mencoba OpenRouter...", "warn")
            smGenStatus.Text = "🟣 Mencoba OpenRouter..."
            task.wait(0.2)
            sheet, genErr = generateWithOpenRouter(songName)
        end

        if not sheet and apiKeys.gemini ~= "" then
            addLog("⚠ OpenRouter: " .. (genErr or "?") .. " — mencoba Gemini...", "warn")
            smGenStatus.Text = "🔵 Mencoba Gemini..."
            task.wait(0.2)
            sheet, genErr = generateWithGemini(songName)
        end
    end
    ...
    if sheet and sheet ~= "" then
        smOutBox.Text       = sheet
        smOutBox.TextColor3 = C.IVORY
        addLog("✅ Sheet dibuat! (" .. #sheet .. " karakter)", "success")
        notify("Sheet Berhasil!", "Tekan '▶️ Gunakan Sheet Ini' untuk mulai main!", 5, "success")
    else
        smOutBox.Text       = "❌ Gagal: " .. (genErr or "Semua API gagal. Cek key & koneksi.")
        smOutBox.TextColor3 = C.RED
        addLog("❌ Gagal: " .. (genErr or "?"), "warn")
        notify("Gagal Generate", genErr or "Cek API key dan koneksi internet.", 5, "error")
    end
